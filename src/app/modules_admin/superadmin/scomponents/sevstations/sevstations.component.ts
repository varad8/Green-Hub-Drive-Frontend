...
@Component({
  selector: 'app-sevstations',
  templateUrl: './sevstations.component.html',
  styleUrl: './sevstations.component.css',
})
export class SevstationsComponent {
  stationdetails: any;
  countdata: any;
  bookingsData: any;
  session: any;
  ...
  ngOnInit(): void {
    ...
    this.getBookings(this.stationid);
    this.getPaymentsData(this.stationid);
    this.getCountData(this.stationid);
    this.getStationDetails(this.stationid);
  }

  getStationDetails(stationid: string) {
    this.auth.getAdminProfileUsingId(stationid).subscribe(
      (data) => {
        this.stationdetails = data[0];
      },
      (error) => {
  ...
  getCountData(userId: string): void {
    this.evdata.getCountDatat(userId).subscribe(
      (data: any) => {
        // Handle data here
        this.countdata = data;
  ...
  getBookings(userId: string): void {
    this.evdata.getChartBookingData(userId).subscribe(
      (data: any) => {
        // Handle data here
  ...
  getPaymentsData(userId: string): void {
    this.evdata.getChartPaymentData(userId).subscribe(
      (data: any) => {
        // Handle data here
